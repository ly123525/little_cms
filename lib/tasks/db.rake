...
namespace :db do
  namespace :backup do
    ...
    def makeup_backup_filename(magic_date)
      "#{Rails.env}#{magic_date.strftime("%Y%m%d%H%M%S")}.sql"
    end
    ...
    task :destroy => [:environment] do
      ...
      (dir.entries - ['.', '..']).each do |backup|
        if backup.starts_with?(Rails.env) && backup < magic_name
    ...
    task :rebuild => [:environment] do
      raise 'only for development or test environment' unless ["development", "test"].include? Rails.env
      backup_folder = makeup_backup_folder
      db_config = ActiveRecord::Base.configurations[Rails.env]
      ...
      (Dir.new(backup_folder).entries - ['.', '..']).sort.reverse.each do |backup|
        (bakfile = backup and break) if backup.starts_with?(Rails.env)
      end unless bakfile
